---
title: "ProcRoySocB initial submission analysis"
...
#100 is low, but we are not actually using these (much)beyond a quick look) and it speeds up the code
mc.trials=1000
```
...
```{r}
dyad.data <- load.data.by.prefix("D", "../../Data/NoBottleneckData/Dyads")
```
```{r}
pseudodyad.data <- load.data.by.prefix("P",  "../../Data/NoBottleneckData/PseudoDyads")
```

```{r}
single.data <- load.data.by.prefix("S",  "../../Data/NoBottleneckData/Singles")
```
...
ggplot(data=proportion.data) +
  ...
  stat_summary(aes(x=Stage, y=MajorityParticleUse),geom='point', fun.y='mean', colour='black',fill='black',size=3, shape=23) +
  stat_summary(aes(x=Stage, y=MajorityParticleUse),geom='errorbar', fun.data='mean_cl_boot',fun.ymin="min", fun.ymax="max",width=0.2) +
...
```{r}
proportion.model <- glmer(MajorityParticleUse ~ Condition * Stage * NCategories + (1 + Stage | ParticipantID:Chain),
...
```{r}
proportion.model.s <- glmer(MajorityParticleUse ~ ConditionSinglesRef * Stage * NCategories + (1 + Stage | ParticipantID:Chain),
                              data=output.data.for.analysis, family=binomial, control=glmerControl(optimizer="bobyqa"))
...
```{r}
proportion.model.p <- glmer(MajorityParticleUse ~ ConditionPseudosRef * Stage * NCategories + (1 + Stage | ParticipantID:Chain),
                              data=output.data.for.analysis, family=binomial, control=glmerControl(optimizer="bobyqa"))
...
```{r}
proportion.model.d <- glmer(MajorityParticleUse ~ ConditionDyadsRef * Stage * NCategories + (1 + Stage | ParticipantID:Chain),
                              data=output.data.for.analysis, family=binomial, control=glmerControl(optimizer="bobyqa"))
summary(proportion.model.d)
```

For the dyads, is the effect of stage significant even if you just look at the 2-category guys? The models above suggest the effect is about half as big as in the 1-category condition. We can check by using treatment coding on NCategories too, with Two Categories set as the reference level. This model still shows a significant effect of Stage, i.e. the 2-category dyads *are* more regular at interaction.
...
proportion.model.d.2 <- glmer(MajorityParticleUse ~ ConditionDyadsRef * Stage * NCategories2Ref + (1 + Stage | ParticipantID:Chain),
                              data=output.data.for.analysis, family=binomial, control=glmerControl(optimizer="bobyqa"))
...
Scrambing lexical conditioning while preserving class conditioning is easy - to generate a randomisation we simply randomise the particule assignment to nouns within each class, which leaves the distribution within each class (and therefore the class-based conditioning) unaltered.
...
Plot total entropy - no point running a stat on this since it's the same as the proportion data.

```{r}
ggplot(data=subset(entropy.data,Measure=="IndividualTotalEntropy")) +
  facet_grid(NCategories~Condition) +
  stat_summary(aes(x=Stage, y=Entropy),geom='point', fun.y='mean', colour='black',fill='black',size=3, shape=23) +
  stat_summary(aes(x=Stage, y=Entropy),geom='errorbar', fun.data='mean_cl_boot',fun.ymin="min", fun.ymax="max",width=0.2) +
...
ggplot(data=entropy.data.class.mi.combined) +
  facet_grid(NCategories~Condition) +
  stat_summary(aes(x=Stage, y=ClassMI),geom='point', fun.y='mean', colour='black',fill='black',size=3, shape=23) +
  stat_summary(aes(x=Stage, y=ClassMI),geom='errorbar', fun.data='mean_cl_boot',fun.ymin="min", fun.ymax="max",width=0.2) +
...
entropy.data.for.analysis$Stage <- droplevels(entropy.data.for.analysis$Stage)
...
```{r}
class.mi.model <- lmerTest::lmer(Entropy ~ Condition * Stage + (1 | Participant:Chain),
                              data=subset(entropy.data.for.analysis,Measure=='IndividualMutualInformationClass' & NCategories=="Two Categories"))

# I had problems installing the necessary libraries to have this run
#class.mi.model.br <- glmmTMB(Entropy ~ Condition * Stage + (1 | Participant:Chain),
#data=subset(subset(entropy.data.for.analysis,Measure=='IndividualMutualInformationClass' & NCategories=="Two Categories"),Entropy<1),
#ziformula=~ Condition * Stage + (1 | Participant:Chain),
#family=beta_family())
```
...
ggplot(data=entropy.data.lexical.mi.combined) +
  facet_grid(NCategories~Condition) +
  stat_summary(aes(x=Stage, y=LexicalMI),geom='point', fun.y='mean', colour='black',fill='black',size=3, shape=23) +
  stat_summary(aes(x=Stage, y=LexicalMI),geom='errorbar', fun.data='mean_cl_boot',fun.ymin="min", fun.ymax="max",width=0.2) +
...
```{r}
lexical.mi.model <- lmerTest::lmer(Entropy ~ Condition * Stage * NCategories + (1 | Participant:Chain),
                              data=subset(entropy.data.for.analysis,Measure=='IndividualMutualInformationLexical'))
```
...
```{r}
lexical.mi.model.notfull <- lmerTest::lmer(Entropy ~ Condition * Stage * NCategories + (1 | Participant:Chain),
...
```{r}
lexical.mi.model.nothigh <- lmerTest::lmer(Entropy ~ Condition * Stage * NCategories + (1 | Participant:Chain),
...
This model (where everything is treatmemt coded) shows no effect of Stage or NCategories but a Stage*NCategories interaction that shows that lexical (!) MI increased in interaction in Two-Category dyads but not one-category dyads.
...
for (p in levels(all.data.for.priming.analysis$Chain)) {
  ...
  this.pair.data$ParticleRecoded <- mapply(function (n,particle) ifelse(particle == 0,"NoParticle",ifelse(particle==this.pair.training.particles[1],"Particle1",ifelse(particle == this.pair.training.particles[2],"Particle2",NA))),
...
Exclude trials where participant *or partner's prime* was an illegal description. Counts suggest there are no such cases? **need to check**
...
ggplot(data=aggregated.priming.data) +
  facet_grid(NCategories~Condition) +
  stat_summary(aes(x=PartnerLastParticle, y=ParticleRecodedBinary),geom='point', fun.y='mean', colour='black',fill='black',size=3, shape=23) +
  stat_summary(aes(x=PartnerLastParticle, y=ParticleRecodedBinary),geom='errorbar', fun.data='mean_cl_boot',fun.ymin="min", fun.ymax="max",width=0.2) +
...
```{r}
priming.model <- glmer(ParticleRecodedBinary ~ Condition * NCategories * PartnerLastParticle + (1 + PartnerLastParticle | ParticipantID:Chain),
                              data=priming.data.for.analysis, family=binomial, control=glmerControl(optimizer="bobyqa"))
```

Model shows effects of:
- PartnerLastParticle (there is priming - if partner produced particle 2 you are more likely to do so)
- condition x partner last particle: a larger priming effect in dyads
- condition2 x n categories (less use of particle2 overall in the 2-categories dyads? That's a bit weird, as far as I can work out it's arbitrary (you still get it e.g. if you assign a random particle as particle 2)
...
Plot (not in paper). NB flipping the layout of the grid to make it easier to compare within conditions.
...
```{r}
#singular fit
#priming.model.2category <- glmer(ParticleRecodedBinary ~ Condition * PrimeOfSameCategory * PartnerLastParticle + (1 + PartnerLastParticle * PrimeOfSameCategory | ParticipantID:Chain), data=subset(priming.data.for.analysis,NCategories=="Two Categories"), family=binomial, control=glmerControl(optimizer="bobyqa"))

#singular fit, simplifying
#priming.model.2category <- glmer(ParticleRecodedBinary ~ Condition * PrimeOfSameCategory * PartnerLastParticle + (1 | ParticipantID:Chain) + (0 + PartnerLastParticle * PrimeOfSameCategory | ParticipantID:Chain),data=subset(priming.data.for.analysis,NCategories=="Two Categories"), family=binomial, control=glmerControl(optimizer="bobyqa"))

#singular fit, simplifying
#priming.model.2category <- glmer(ParticleRecodedBinary ~ Condition * PrimeOfSameCategory * PartnerLastParticle + (1 + PartnerLastParticle + PrimeOfSameCategory | ParticipantID:Chain), data=subset(priming.data.for.analysis,NCategories=="Two Categories"), family=binomial, control=glmerControl(optimizer="bobyqa"))

priming.model.2category <- glmer(ParticleRecodedBinary ~ Condition * PrimeOfSameCategory * PartnerLastParticle + (1 + PartnerLastParticle  | ParticipantID:Chain), data=subset(priming.data.for.analysis,NCategories=="Two Categories"), family=binomial, control=glmerControl(optimizer="bobyqa"))

```

This model shows a Condition2:PrimeOfSameCategory1 interaction - not sure that is interpretable though, it just means they use particle 2 less on trials where the primes are of the same category (i.e. regardless of the prime particle). Crucially there is no PrimeOfSameCategory1:PartnerLastParticle1 interaction (which would suggest stronger priming within- than across-category).
...
Looks like there's clearly a huge effect of same-noun primes (the effect of the partner particle choice is much bigger on same noun trials), and this effect seems bigger in dyads. NB the number of trials in SameNounPrime is very small.
...
```{r}
#singular fit
priming.model.noun <- glmer(ParticleRecodedBinary ~ Condition * NCategories * PrimeOfSameNoun * PartnerLastParticle + (1 + PartnerLastParticle * PrimeOfSameNoun | ParticipantID:Chain), data=priming.data.for.analysis, family=binomial, control=glmerControl(optimizer="bobyqa"))

```

There is a clear lexical boost (as indicated by a significant PrimeOfSameNoun:PartnerLastParticle interaction). There are several marginal interactions involving condition2 - looks like the lexical boost is marginally bigger in dyads than in singles/pseudos (Condition2:PrimeOfSameNoun:PartnerLastParticle).
